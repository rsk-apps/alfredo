...
func New(cfg Config) (*echo.Echo, error) {
	...
	healthApptRepo := healthsql.NewHealthAppointmentRepository(cfg.DB)
	...
	healthApptService := healthsvc.NewHealthAppointmentService(healthApptRepo)
	...
	healthApptUC := app.NewHealthAppointmentUseCase(healthApptService, healthProfileService, cfg.Calendar, cfg.Telegram, cfg.Location.String(), logger)
	...
	healthApptHandler := healthhttp.NewHealthAppointmentHandler(healthApptUC, cfg.Location)
	...
	healthApptHandler.Register(protected)
